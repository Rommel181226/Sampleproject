...
st.title("\U0001F5C2️ Task Time Analysis Dashboard")

# Sidebar - Logo and Title
logo_path = os.path.join("images", "logo.png")
if os.path.exists(logo_path):
    st.sidebar.image(logo_path, width=150)
st.sidebar.markdown("## \U0001F4C1 Task Dashboard Sidebar")
...
if uploaded_files:
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs([
        "Summary Tables", "Dashboard KPIs", "Charts", "Task Types",
    ...
    with tab3:
        st.markdown("### \U0001F4CA Time Spent per User")
        ...
        st.markdown("### \U0001F5D3️ Time Distribution by Date")
        ...
        st.markdown("### \U0001F4CB Task Records")
        st.dataframe(filtered_df[['date', 'user_first_name', 'user_last_name', 'task', 'minutes']], use_container_width=True)
        st.download_button("\U0001F4C5 Download Filtered Data", data=filtered_df.to_csv(index=False), file_name="filtered_data.csv")
    ...
    with tab7:
        st.subheader("\U0001F4C5 Calendar Heatmap")
